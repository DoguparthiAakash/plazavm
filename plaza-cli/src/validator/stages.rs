...
use plaza_config::WorkspaceConfig;
use plaza_core::paths;
use plaza_core::types::OperatingSystem;
use plaza_events::PlazaEvent;
use plaza_workspace::model::{DesiredState, RuntimeKind, WorkspaceSpec, WorkspaceState};
use std::sync::Arc;
...
impl StageExecutor {
    ...
    pub async fn stage_5_failure_injection(collector: &EvidenceCollector) -> StageResult {
        ...
        let fetched = container
            ...
            .unwrap()
            .unwrap();
        let res = container.controller.reconcile_workspace(&fetched).await;
    ...
    pub async fn stage_6_decision_matrix(collector: &EvidenceCollector) -> StageResult {
        ...
        let mut container_spec = WorkspaceSpec::default();
        container_spec.runtime.kind = RuntimeKind::Container;
        let dec1 = container
            .decision_engine
            .decide(container_spec)
            .await
            .unwrap();

        let mut vm_spec = WorkspaceSpec::default();
        vm_spec.runtime.kind = RuntimeKind::VirtualMachine;
        vm_spec.runtime.os = OperatingSystem::Windows;
        let dec2 = container.decision_engine.decide(vm_spec).await.unwrap();
        ...
        let details = vec![
            format!(
                ...
                dec1.selected_backend.backend_id, dec1.selected_backend.reason
            ),
            format!(
                "VirtualMachine Spec -> Selected Backend: '{}' (Reasoning: {})",
                dec2.selected_backend.backend_id, dec2.selected_backend.reason
            ),
        ];
    ...
    pub async fn stage_8_plugin_validation(collector: &EvidenceCollector) -> StageResult {
        ...
        let bus = Arc::new(plaza_events::EventBus::new());
    ...
    pub async fn stage_9_security_audit(collector: &EvidenceCollector) -> StageResult {
        ...
        let sec_json = serde_json::json!({
            ...
            "path_traversal_sandbox": "plaza_core::paths sandbox bounded",
